...
public class BbsDetailActivity extends BaseActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btnSend:
                SharedPreferences loginInfo = getSharedPreferences(Constants.SHARE_USER_INFO, MODE_PRIVATE);
                if (loginInfo.getString("userName","").equals("")) {
                    MyLog.i(TAG, "no login information ,need jump to login ");
                    Intent intent = new Intent(BbsDetailActivity.this, LoginActivity.class);
    ...
    private void setAnswerModel(int model,String toName){
        ...
        etContent.setHint("reply" + toName);//被回复用户名
    ...
    private void recoverEditText(){
        answerModel = 0;
        etContent.setText("");
        etContent.setHint("Reply to the landlord\n" +
                "\n");
    }
            ...
            public void onResponse(Call<com.lei.bbs.bean.Response> call, Response<com.lei.bbs.bean.Response> response) {
                if (response.body() != null){
                    if (response.body().getStatus().equals("1")){
                        MyToast.showShort(BbsDetailActivity.this, "Reply successful\n" +
                                "\n");
                        swipeRefreshLayout.setRefreshing(true);
                        requestListViewData();
                        recoverEditText();
    ...
    private void sendAnswerFeed(int mid,int uid,String content){
        ...
        sendFeed.enqueue(new Callback<com.lei.bbs.bean.Response>() {
            ...
            @Override
            public void onFailure(Call<com.lei.bbs.bean.Response> call, Throwable t) {
                MyToast.showShort(BbsDetailActivity.this, "Please try again later\n" +
                        "\n");
            }
        });
    }
